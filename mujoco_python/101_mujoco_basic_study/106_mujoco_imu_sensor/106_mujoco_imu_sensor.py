...
while not glfw.window_should_close(window):
    ...
    #print("센서 개수:", model.nsensor)
    #print("센서 이름:", [model.sensor(i).name for i in range(model.nsensor)])

        # 센서값 읽기: IMU (accelerometer + gyroscope)
    try:
        acc_id = model.sensor(name='imu_acc').id
        gyro_id = model.sensor(name='imu_gyro').id
       
        acc_data = data.sensordata[acc_id:acc_id+3]   # x, y, z
        gyro_data = data.sensordata[gyro_id:gyro_id+3]  # x, y, z
